...
function addArrayMethods (nativeMethods, customMethods) {
  var allMethods = nativeMethods.map(function(method){
    var obj = {}
    obj[method] = new Function(
      'return this._addArrayMethods([].'+method+'.apply(this, arguments))'
    )
    return obj
  }).concat(customMethods)
  return function(arr) {
    var self = this
    arr._addArrayMethods = self._addArrayMethods
    allMethods.forEach(function(kv){
      for(let method in kv) {
        arr[method] = kv[method]
      }
    })
    return arr
  }
