...
function queryVnode (vnode, query, store) {
  ...
  if(query.level-- > 0) {
    queryVnode(vnode.children, query, store)
  }
  return store
}
...
export default function bindM (M) {
  ...
  const factory = function (cssobjResult) {
    ...
    c.queryVnode = function(vnode, query, options) {
      let store = []
      if(typeof query!='string') return store
      options = options || {}
      let parts = getSelectorParts(query)
      let first = true
      if (options.childOnly) {
        let children = []
        ;[].concat(vnode).forEach(function(v){
          if(Array.isArray(v.children)) children = children.concat(v.children)
        })
        vnode = children
      }
      for(let key in parts) {
        ...
        selector.pseudo = key
        selector.level = first 
          ? 'level' in options 
            ? options.level|0
            : Infinity 
          : 0
        vnode = queryVnode(vnode, selector)
        first = false
      }
      return vnode
    }
    ...
    c.query = function(vnode, query, options){
      var nodes = []
      c.queryVnode(vnode, query, options).forEach(function(v){
